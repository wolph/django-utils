...
- Type — keys, strings, numbers and `true`/`false`/`null` are
  colorized live. The highlighting is a ~30-line tokenizer shipped in
...
- Delete the closing `}` (or otherwise break the JSON) — an inline
  ...
  `JSON.parse()` message. The colors keep working on broken JSON: the
  highlighter lexes, it doesn't parse.
